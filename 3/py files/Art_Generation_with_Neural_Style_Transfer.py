...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(a_S, shape=[m, -1, n_C])[0,:], perm=[1,0])
    a_G = tf.transpose(tf.reshape(a_G, shape=[m, -1, n_C])[0,:], perm=[1,0])
...
content_image = scipy.misc.imread("images/Aviv-34-small.jpeg")
# content_image=scipy.ndimage.rotate(content_image, 90)
...
style_image = scipy.misc.imread("images/MyStyle.jpeg")
...
### START CODE HERE ### (1 line)
J = total_cost(J_content, J_style, alpha = 100, beta = 10)
### END CODE HERE ###
...
generated_image = scipy.misc.imread("images/generated_image-2nd.png")
print(generated_image.shape)
generated_image = reshape_and_normalize_image(generated_image)
model_nn(sess, generated_image, 300)

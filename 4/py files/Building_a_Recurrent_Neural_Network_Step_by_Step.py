...
def rnn_cell_backward(da_next, cache):
    ...
    dxt = np.dot(np.transpose(Wax), (da_next * (1 - (dz**2))))
    dWax = np.dot((da_next * (1 - (dz**2))), np.transpose(xt))

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.dot(np.transpose(Waa), (da_next * (1 - (dz**2))))
    dWaa = np.dot((da_next * (1 - (dz**2))), np.transpose(a_prev))
